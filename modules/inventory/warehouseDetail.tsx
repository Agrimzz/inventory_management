...
const WarehouseDetail = () => {
  ...
  const { data, loading, error, refetch } = useFetch<WarehouseWithIdSchema>(
    `/warehouses/${id}/`,
    true
  );
  ...
  const warehouseImages = data?.images || [];
    ...
    <SafeAreaView className="bg-background  flex-1 pb-4 relative">
      ...
      <View className="w-full flex-row items-center justify-between p-4 z-20">
        ...
        <View className="flex-row gap-2">
          <Pressable
            onPress={() =>
              router.push(`/form/edit/warehouse/${data?.id}` as any)
            }
      ...
      <View className="absolute top-0 left-0 w-full">
        ...
        {warehouseImages.length > 1 ? (
          ...
          <Image
            source={{ uri: warehouseImages[0].upload }}
      ...
      <ScrollView className="flex-1 mt-64 w-full bg-gray rounded-3xl p-6">
